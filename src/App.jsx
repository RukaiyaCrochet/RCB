            ...
            {filteredProducts.length > 0 ? (
              filteredProducts.map((product) => {
                ...
                <div key={product.id} className="relative animate-fade-in flex flex-col h-full">
                  ...
                  {/* Image Container */}
                  <div className="aspect-[3/4] sm:aspect-[4/5] bg-stone-100 rounded-2xl sm:rounded-3xl overflow-hidden mb-4 sm:mb-5 relative shadow-md hover:shadow-xl transition-all duration-500 flex-shrink-0 group">
                    <img 
                      ...
                      // loading="lazy"
                      // decoding="async"
                      // onClick={() => setShowProductOverlay({...showProductOverlay, [product.id]: !showProductOverlay[product.id]})}
                      // className={`w-full h-full ${product.id === 17 | 2 | 1? 'object-contain' : 'object-cover'} transition-transform duration-700 hover:scale-110 cursor-pointer`}
                      ...
                      loading="lazy"
                      decoding="async"
                      className={`w-full h-full object-contain object-cover   p-2 transition-transform duration-700 hover:scale-105`}
                      sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw"
                    />
                    ...
                    {images.length > 1 && (
                      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 flex justify-center gap-2">
                        {images.map((_, idx) => (
                          <button
                            key={idx}
                            onClick={() => setProductImageIndex({...productImageIndex, [product.id]: idx})}
